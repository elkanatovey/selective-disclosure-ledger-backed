...
build_cli() {
    ...
    if command -v cmake >/dev/null 2>&1 && [ -d /opt/ccf_virtual ]; then
        ...
        local helper
        helper=$(docker create --entrypoint "" "${IMAGE_TAG}" true)
        docker cp "${helper}:/opt/scitt-sd/bin/scitt-sd" "${REPO_ROOT}/build/scitt-sd.bin"
        docker cp "${helper}:/opt/scitt-sd/lib" "${REPO_ROOT}/build/lib"
        docker rm "${helper}" >/dev/null
        cat >"${REPO_ROOT}/build/scitt-sd" <<EOF
#!/bin/sh
# Generated by demo/run.sh. Runs the tool extracted from ${IMAGE_TAG}.
LD_LIBRARY_PATH="${REPO_ROOT}/build/lib\${LD_LIBRARY_PATH:+:\$LD_LIBRARY_PATH}"
export LD_LIBRARY_PATH
exec "${REPO_ROOT}/build/scitt-sd.bin" "\$@"
EOF
