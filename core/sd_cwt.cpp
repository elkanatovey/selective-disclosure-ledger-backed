...
namespace sdcwt
{
  ...
  {
    // Reject a bad protected header (duplicate label) before any redaction.
    auto phdr =
    ...
    PreparedToken out;
    out.payload = encode_value(redacted);
    out.to_be_signed = cose_to_be_signed(phdr, out.payload);
    out.protected_header = std::move(phdr);
    ...
    const HeaderEntries& extra_protected)
  {
    ...
    auto prepared = detail::prepare(
      claims, redact_paths, cose_alg, sd_alg, rng, pad_to, extra_protected);

    IssuedToken out;
    out.token =
      sign_cose_sign1(key, prepared.protected_header, prepared.payload);
    out.disclosures = std::move(prepared.disclosures);
    return out;
  }
    ...
    const std::vector<std::vector<uint8_t>>& items)
  {
    ...
    // Rebuild the COSE_Sign1, replacing only the unprotected-header entry
    // named by `label`. Protected header, payload and signature are re-emitted
    // unchanged so the signature stays valid, and every other unprotected
    // entry is carried over: a receipt (394) survives a disclosure edit (17),
    // and a disclosure set survives receipt attachment.
    cbor::Value root;
    ...
    {
      throw std::runtime_error("malformed COSE_Sign1 token");
    }
    ...
    {
      throw std::runtime_error("malformed unprotected header");
    }

    std::vector<cbor::MapItem> uhdr;
    for (const auto& [entry_label, value] :
         std::get<cbor::Map>(parts[1]->value).items)
    {
      const bool is_target =
        std::holds_alternative<cbor::Signed>(entry_label->value) &&
        entry_label->as_signed() == label;
      if (!is_target)
      {
        uhdr.emplace_back(entry_label, value);
      }
    }
    if (!items.empty())
    {
      std::vector<cbor::Value> encoded;
      encoded.reserve(items.size());
      for (const auto& item : items)
      {
        encoded.push_back(bytes_value(item));
      }
      uhdr.emplace_back(
        cbor::make_signed(label), cbor::make_array(std::move(encoded)));
    }
  ...
  {
    return set_unprotected_bstr_array(token, SD_CLAIMS_LABEL, selected);
  }
}
