...
namespace scitt_sd::cli
{
    ...
    std::string_view document, int64_t issued_at)
  {
    ...
    if (!parsed.is_object())
    {
      throw UsageError("the report must be a JSON object");
    }
    for (const auto& [key, unused] : parsed.items())
    {
      ...
      {
        throw UsageError("the report carries an unknown field '" + key + "'");
      }
    }
    ...
    {
      throw UsageError("the report field 'references' must be an array");
    }
    ...
    for (const auto& reference : references)
    {
      ...
      {
        throw UsageError("every report reference must be a string");
      }
      auto text = reference.get<std::string>();
      if (text.find('\0') != std::string::npos)
      {
        throw UsageError("a report reference contains a NUL character");
      }
      size_t chars = 0;
      try
      {
      ...
      {
        throw UsageError("a report reference is not well-formed UTF-8");
      }
      if (chars == 0)
      {
        throw UsageError("a report reference must not be empty");
      }
