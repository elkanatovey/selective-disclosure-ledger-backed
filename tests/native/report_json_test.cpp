...
{
  EXPECT_THROW((void)parse_report_json("{", ISSUED_AT), UsageError);
}

TEST(ReportJson, RefusesANonObject)
{
  EXPECT_THROW((void)parse_report_json("[]", ISSUED_AT), UsageError);
  EXPECT_THROW((void)parse_report_json("\"text\"", ISSUED_AT), UsageError);
  EXPECT_THROW((void)parse_report_json("null", ISSUED_AT), UsageError);
}
...
{
  EXPECT_THROW((void)parse_report_json(document("42"), ISSUED_AT), UsageError);
  EXPECT_THROW(
    (void)parse_report_json(document("null"), ISSUED_AT), UsageError);
  EXPECT_THROW(
    (void)parse_report_json(document(R"("t")", "true"), ISSUED_AT), UsageError);
  EXPECT_THROW(
...
{
  EXPECT_THROW(
    (void)parse_report_json(document(R"("")"), ISSUED_AT), UsageError);
  EXPECT_THROW(
...
TEST(ReportJson, RefusesAnEmbeddedNul)
{
  ...
  EXPECT_THROW((void)parse_report_json(with_nul, ISSUED_AT), UsageError);
}
...
TEST(ReportJson, CountsLimitsInCodePointsNotBytes)
{
  ...
  EXPECT_THROW(
    (void)parse_report_json(document(over_limit), ISSUED_AT), UsageError);
}
...
TEST(ReportJson, RefusesAnOverlongBody)
{
  ...
  EXPECT_THROW(
    (void)parse_report_json(document(R"("t")", body), ISSUED_AT), UsageError);
}
