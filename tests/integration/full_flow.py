...
"""The whole demo, driven through the app's own HTTP API.

This exercises the path a browser takes: enrol a key, sign the prepared bytes,
register with a real transparency service, withhold part of the report, sign the
release, then verify it against trust material supplied out of band.
...
the application itself never sees one.
"""
...
APP = os.environ.get("APP_URL", "http://127.0.0.1:8090").rstrip("/")
TIMEOUT = 120
...
def get(path: str) -> bytes:
    with urllib.request.urlopen(APP + path, timeout=TIMEOUT) as response:
...
def main() -> None:
    ...
    post("/api/msrc/key", {"public_key_pem": msrc_public})
    ...
    print(f"registered as {txid} with {submitted['scitt_url']}")

    bundle = get(f"/api/submissions/{submitted['submission_id']}/bundle")
    release = post(
        "/api/msrc/release",
        {
    ...
    root_pem = get("/api/msrc/root").decode()
    service_pem = get("/api/scitt/certificate").decode()
